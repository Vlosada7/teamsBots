    ...
    switch (message.text) {
      ...
      case "help":
        response =
          ...
          "- 'download ISO': Download the emails. \n" +
          "- 'login': Authenticate to access your email.\n" +
          "\nFeel free to ask for help anytime you need it!";
        break;
      case "login":
        const authUrl = `${process.env.REDIRECT_URI}/auth`; // Define the auth endpoint
        response = `Click [here](${authUrl}) to log in and authenticate.`;
        break;
      default:
